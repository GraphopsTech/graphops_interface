...
def parse_args() -> argparse.Namespace:
    ...
    p_scan = sub.add_parser(
        "scan",
        help="Scan Ruby project via ruby_base. Writes protobuf batches and uploads unless --no-upload is set.",
    )
    p_scan.add_argument("--path", "-p", help="Override root path (default from graphops.yml root_path).")
    p_scan.add_argument("--rules-dir", "-r", help="Path to rules directory (e.g. backend/rules).")
    p_scan.add_argument("--exclude", action="append", metavar="DIR", help="Exclude directory (repeatable).")
    p_scan.add_argument("--output", "-o", help="Output base path (default: output/nodes.pb -> output/nodes_batches/*.pb.gz).")
    p_scan.add_argument("--backend-url", "-b", help="Backend URL for rules and upload.")
    p_scan.add_argument("--validate-ids", action="store_true", help="Validate all IDs during extraction.")
    p_scan.add_argument("--no-upload", action="store_true", help="Skip uploading to backend.")
